...
def normalize_to_annual(
    ...
    if isinstance(value, tuple):
        return normalize_range(value, price_type, hours_per_year)
...
def normalize_range(
    ...
    if min_val is not None:
        min_val = normalize_to_annual(min_val, price_type, hours_per_year)
    
    if max_val is not None:
        max_val = normalize_to_annual(max_val, price_type, hours_per_year)
...
def estimate_from_type(price_type: str, hours_per_year: int = HOURS_PER_YEAR) -> Optional[float]:
    """
