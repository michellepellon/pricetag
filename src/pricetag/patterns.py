...
NUMBER = rf'(?:{NUMBER_WITH_COMMAS}|{NUMBER_NO_COMMAS}|{ABBREVIATED_NUMBER})'
...
# Range patterns
RANGE_WITH_SYMBOL = rf'{CURRENCY_SYMBOL}\s*{NUMBER}{RANGE_SEPARATOR}{CURRENCY_SYMBOL}?\s*{NUMBER}'
...
BETWEEN_PATTERN = rf'(?:between|from)\s+{SINGLE_AMOUNT}\s+(?:and|to|through)\s+{SINGLE_AMOUNT}'
...
def estimate_from_contextual(term: str, modifier: str = None) -> tuple[float, float] | None:
    """
...
def extract_range_values(match_text: str, separator_pattern: str = None) -> tuple[float | None, float | None] | None:
    """
    ...
    if len(numbers) < 1:
        ...
        if re.search(r'(?:up\s+to|maximum|max|no\s+more\s+than)', match_text, re.IGNORECASE):
            return (None, value)
        # Check for "starting at", "minimum" patterns
        elif re.search(r'(?:starting|from|minimum|min|at\s+least|no\s+less\s+than)', match_text, re.IGNORECASE):
            return (value, None)
...
def infer_range_from_context(text: str, value: float) -> tuple[float | None, float | None] | None:
    """
    ...
    # "up to X" patterns
    if any(phrase in text_lower for phrase in ['up to', 'maximum', 'max of', 'no more than']):
        return (None, value)
    
    # "starting at X" patterns
    if any(phrase in text_lower for phrase in ['starting at', 'from', 'minimum', 'min of', 'at least', 'no less than']):
        return (value, None)
    ...
    if any(phrase in text_lower for phrase in ['approximately', 'about', 'around', 'circa']):
...
def has_price_indicators(text: str) -> bool:
    ...
    if re.search(r'\d{1,3},\d{3}', text):
        return True
    if re.search(r'(?:salary|compensation|pay|wage|hour|annual|month)', text, re.IGNORECASE):
        return True
    return False


def find_numeric_prices(text: str, max_results: int = None) -> list[dict]:
    """
...
def find_contextual_prices(text: str) -> list[dict]:
    """
